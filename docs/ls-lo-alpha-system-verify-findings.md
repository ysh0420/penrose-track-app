...
- 必要 env: **`JQUANTS_API_KEY` のみ**。`penrose-research-engine/.env` に設定済（Brainの762銘柄日次取込が同キーで稼働中）。
- ⚠️ セキュリティ: `penrose-secrets-backup/.env.bak-before-bom-fix` に JQUANTS の **平文認証情報**（EMAIL/PASSWORD/API_KEY）が存在。本ドキュメント・コミット・メモリには値を一切記録しない。EMAIL/PASSWORD は v1 レガシーで v2 では不使用。
- ⚠️ prdj の edge function secrets に `JQUANTS_API_KEY` が設定済かは **未確認**（`env-check` 関数は SUPABASE_* しか probe しない）。Option B を採る場合は prdj への secret 追加が必要。
